...
class MemoryWriteTool(Tool):
    ...
    def run(self, **kwargs: Any) -> str:
        from crucible.memory.models import Entry
...
class WebSearchTool(Tool):
    ...
    Off by default (``CRUCIBLE_WEB_SEARCH_ENABLED=true`` to enable) because network
    ...
    def _search_instant_answer(self, query: str, limit: int) -> list[dict[str, str]]:
        ...
        try:
            request = urllib.request.Request(url, headers={"User-Agent": "crucible/0.2"})
