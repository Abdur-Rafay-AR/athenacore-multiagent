...
Install with ``pip install 'crucible-agents[api]'`` and start with ``crucible serve``.
...
try:
    ...
    raise ImportError(
        "The API needs extra packages. Install with: pip install 'crucible-agents[api]'"
    ) from exc
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    def create_run(
        ...
        except (ConfigurationError, CrucibleError) as exc:
    ...
    def stream_run(payload: RunRequest, store: SqliteMemoryStore = Depends(get_store)):
        ...
        worker = threading.Thread(target=work, name="crucible-api-run", daemon=True)
        worker.start()
...
app = None  # populated lazily by `crucible serve`
