...
class HumanFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        name = record.name.removeprefix("crucible.")
        extras = " ".join(
            f"{k}={v}"
...
def configure_logging(
    ...
    """Install a single stderr handler on the ``crucible`` logger tree."""
    global _CONFIGURED
    root = logging.getLogger("crucible")
...
def get_logger(name: str) -> logging.Logger:
    ...
    if not name.startswith("crucible"):
        name = f"crucible.{name}"
