...
third-party agents through the ``crucible.agents`` entry-point group. Shipping
...
def _load_plugins() -> None:
    ...
    try:
        ...
        for entry in entry_points(group="crucible.agents"):
            try:
                loaded = entry.load()
...
def _ensure_builtins() -> None:
    ...
    import crucible.agents.builtin  # noqa: F401
...
def describe_agents() -> list[dict[str, Any]]:
    """Metadata for every registered agent - powers ``crucible agents`` and
