...
no dependencies.
    ...
    crucible run "question" --topic energy --preset red-team
    crucible debate "question" --topic energy --rounds 4
    crucible recall "water usage" --topic energy --explain
    crucible topics / timeline / runs / stats / export
    crucible agents / presets / graph / doctor / serve / ui
"""
...
from crucible.logging_setup import configure_logging
...
def _build_orchestrator(settings: Settings, *, quiet: bool, verbose: bool):
    from crucible.orchestration.events import console_printer
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    from crucible.orchestration.presets import build_preset
...
def cmd_topics(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if args.json:
            ...
            return 0
        if not topics:
            print('No topics yet. Start one:  crucible run "your question" --topic mytopic')
            return 0
...
def cmd_agents(args: argparse.Namespace) -> int:
    from crucible.agents.registry import describe_agents
...
def cmd_presets(args: argparse.Namespace) -> int:
    from crucible.orchestration.presets import describe_presets
...
def cmd_doctor(args: argparse.Namespace) -> int:
    ...
    settings = _settings_from_args(args)
    from crucible.llm.registry import build_provider
    ...
    for extra, module, purpose in (
        ("api", "fastapi", "crucible serve"),
        ("ui", "streamlit", "crucible ui"),
        ...
        try:
            ...
            print(
                f"  {extra:<14} not installed {_colour(f'(pip install crucible[{extra}]) for {purpose}', DIM)}"
            )
...
def cmd_serve(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print("The API needs extra packages:  pip install 'crucible-agents[api]'", file=sys.stderr)
        return 1
    from crucible.api.server import create_app

    os.environ.setdefault("CRUCIBLE_DATABASE_PATH", str(settings.database_path))
    print(_heading(f"\nCrucible API on http://{args.host}:{args.port}  (docs at /docs)\n"))
    uvicorn.run(
...
def cmd_ui(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print("The UI needs extra packages:  pip install 'crucible-agents[ui]'", file=sys.stderr)
        return 1
    import subprocess

    app = Path(__file__).parent / "ui" / "app.py"
    os.environ["CRUCIBLE_DATABASE_PATH"] = str(settings.database_path)
    os.environ["CRUCIBLE_MODEL"] = settings.model
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "examples:\n"
            '  crucible run "Is refining the real EV bottleneck?" --topic batteries\n'
            '  crucible run "..." --topic batteries --preset red-team --model ollama:llama3.1\n'
            '  crucible debate "Will sodium-ion displace lithium?" --topic batteries --rounds 4\n'
            '  crucible recall "water usage" --topic batteries --explain\n'
            "  crucible export --topic batteries --format md -o report.md\n"
            "  crucible doctor\n"
        ),
    )
