...
from crucible.errors import ProviderError, ProviderTimeout
...
class LLMProvider(abc.ABC):
    ...
    def health(self) -> tuple[bool, str]:
        """Cheap reachability probe used by ``crucible doctor``."""
        try:
...
def _status_hint(status: int) -> str | None:
    ...
    if status == 429:
        return "Rate limited. Lower CRUCIBLE_MAX_PARALLEL_AGENTS or wait."
    if status >= 500:
