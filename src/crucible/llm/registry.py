...
Turns a spec string plus :class:`~crucible.config.Settings` into a live
provider, so the model backend is a one-line config change everywhere::

    CRUCIBLE_MODEL=ollama:llama3.1          # local, free, default
    CRUCIBLE_MODEL=openai:gpt-4o-mini       # or any OpenAI-compatible endpoint
    CRUCIBLE_MODEL=groq:llama-3.3-70b       # same schema, different base URL
    CRUCIBLE_MODEL=anthropic:claude-sonnet-4-5
    CRUCIBLE_MODEL=echo:test                # offline, deterministic
"""
...
from crucible.config import Settings, split_model_spec
from crucible.errors import ConfigurationError
from crucible.llm.base import LLMProvider
...
# `CRUCIBLE_MODEL=groq:llama-3.3-70b` works without touching a base URL by hand.
...
def register_provider(name: str, factory: ProviderFactory, *, overwrite: bool = False) -> None:
    """Add a provider so ``CRUCIBLE_MODEL=<name>:<model>`` resolves to it."""
