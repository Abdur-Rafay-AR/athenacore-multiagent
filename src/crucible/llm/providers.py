...
all expose ``/chat/completions``. Point ``CRUCIBLE_OPENAI_BASE_URL`` at any of them.
"""
...
from crucible.errors import ProviderError
from crucible.llm.base import (
    Completion,
    LLMProvider,
    Message,
    iter_sse_data,
)
from crucible.memory.models import Usage, estimate_tokens
...
class OllamaProvider(LLMProvider):
    ...
    def connection_hint(self) -> str:
        return (
            ...
            f"Override the endpoint with CRUCIBLE_OLLAMA_HOST (currently {self.host})."
        )
...
class OpenAICompatibleProvider(LLMProvider):
    ...
    def connection_hint(self) -> str:
        return f"Check CRUCIBLE_OPENAI_BASE_URL (currently {self.base_url}) and CRUCIBLE_OPENAI_API_KEY."
    ...
    def _complete(self, messages: Sequence[Message], **options: Any) -> Completion:
        if not self.api_key and "api.openai.com" in self.base_url:
            raise ProviderError(
                ...
                provider=self.name,
                hint="Set CRUCIBLE_OPENAI_API_KEY, or use a local model: CRUCIBLE_MODEL=ollama:llama3.1",
            )
        data = self._post_json(
...
class AnthropicProvider(LLMProvider):
    ...
    def connection_hint(self) -> str:
        return "Set CRUCIBLE_ANTHROPIC_API_KEY, or switch to a local model."
    ...
    def _complete(self, messages: Sequence[Message], **options: Any) -> Completion:
        if not self.api_key:
            raise ProviderError(
                ...
                provider=self.name,
                hint="Set CRUCIBLE_ANTHROPIC_API_KEY.",
            )
        data = self._post_json(
...
class EchoProvider(LLMProvider):
    ...
    Set ``CRUCIBLE_MODEL=echo:test`` to run the whole system offline.
    """
