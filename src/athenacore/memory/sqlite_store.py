...
from athenacore.errors import MemoryError_
...
class SqliteMemoryStore(MemoryStore):
    ...
    def __init__(self, path: str | Path = "data/athenacore.sqlite3") -> None:
        self.path = str(path)
        if self.path != ":memory:":
        ...
        with self._lock:
            ...
            if version > SCHEMA_VERSION:
                raise MemoryError_(
                    ...
                    hint="Upgrade athenacore, or point ATHENA_DATABASE_PATH at a new file.",
                )
            if version < 1:
